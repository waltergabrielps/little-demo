...
test.describe("Trackers Tests", () => {
  // Header of Project
  test(`Trackers on IPs`, async ({ homePage, searchPage }) => {
    ...
    await test.step("The user sees in the Data list an element called 'Trackers on IPs'", async () => {
      await searchPage.checksDivByNameDisplayed("Trackers on IPs");
    });
    await test.step("the user clicks on the 'Trackers on IPs' Tab", async () => {
      await searchPage.clicksOnTab("Trackers on IPs");
    });
    await test.step("Checks the page size is 25", async () => {
      await searchPage.checksPageSize("Trackers on IPs", 25);
    });
    await test.step("Checks the pagination count is matching", async () => {
      await searchPage.checksPaginationIsMatching("Trackers on IPs");
    });
    ...
    await test.step("Sorting by 'Last Seen' is 'descending' by default in 'Trackers on IPs'", async () => {
